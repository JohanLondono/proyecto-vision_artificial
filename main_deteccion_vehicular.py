...
# Configurar TensorFlow y ML libraries automáticamente
import os
import sys
...
# Configuración silenciosa automática desde utils
try:
    from utils.tensorflow_quiet_config import configure_libraries
    configure_libraries()
except ImportError:
    # Configuración manual si utils no está disponible
...
# No configurar backend aquí - se configurará dinámicamente según necesidad
import warnings
...
# Importar sistema de detección con redes neuronales MEJORADO
try:
    ...
    print("🧠 Sistema de Detección con IA MEJORADO disponible")
except ImportError as e:
    NEURAL_DETECTION_AVAILABLE = False
    USING_OLD_SYSTEM = False
    print(f"⚠️  Sistema de Detección con IA MEJORADO no disponible: {e}")
    ...
    try:
        ...
        print("🔄 Usando sistema de IA anterior como fallback")
    except ImportError:
        USING_OLD_SYSTEM = False
        print("❌ Ningún sistema de IA disponible")
...
class SistemaDeteccionVehicular:
    ...
    def configurar_matplotlib_backend(self, interactivo=False):
        """
        Configura el backend de matplotlib según necesidad.
                ...
                try:
                    ...
                    try:
                        ...
                        try:
                            ...
                            print("⚠️ No se encontró backend gráfico disponible")
                            return False
                import matplotlib.pyplot as plt
    ...
    def mostrar_menu_principal(self):
        ...
        print("8. Ayuda")
        print("="*60)
        print("🧠 NUEVAS CAPACIDADES CON REDES NEURONALES:")
        print("🎩 9. Sistema de Detección con IA MEJORADO (Sombreros)")
        print("   🎯 • Selección de modelos interactiva")
        print("   🧠 • Entrenamiento desde cero")
        print("   📹 • Video con configuración avanzada")
        print("   ⚙️  • Configuración completa de parámetros")
        print("="*60)
        print("0. Salir")
        print("="*60)
    ...
    def mostrar_menu_preprocesamiento(self):
        ...
        print("9. Procesar Carpeta de Imágenes (sin previsualización)")
        print("-" * 70)
    ...
    def mostrar_menu_descriptores_textura(self):
        ...
        print("4. Análisis Completo de Texturas")
        print("5. Comparación de Regiones")
        print("0. Volver")
        print("-" * 40)
    ...
    def mostrar_menu_deteccion_bordes(self):
        ...
        print("6. Canny + Estadísticas de Magnitud")
        print("7. Sobel + Estadísticas de Magnitud")
        print("-" * 40)
        print("0. Volver")
        print("-" * 40)
    ...
    def preprocesamiento_por_lotes(self):
        """Maneja el preprocesamiento por lotes de múltiples imágenes."""
        print("\n" + "="*70)
        print("PREPROCESAMIENTO POR LOTES".center(70))
        print("="*70)
        print("Procesamiento automático de múltiples imágenes sin previsualización")
        print("-" * 70)
        
        # Solicitar carpeta de imágenes
        carpeta_default = self.directorio_imagenes
        print(f"Carpeta actual de imágenes: {carpeta_default}")
        ...
        # Seleccionar métodos de preprocesamiento
        ...
        print("Seleccione métodos a aplicar (separados por comas):")
        for key, metodo in metodos_disponibles.items():
            print(f"  {key}. {metodo.replace('_', ' ').title()}")
        
        print("\nPreconfigurados:")
        print("  A. Básico (gaussiano, normalización, CLAHE)")
        print("  B. Completo (básico + bilateral, afilado)")
        print("  C. Automático (solo preprocesamiento automático)")
        
        seleccion = input("\nIngrese números separados por comas o letra preconfigurada: ").strip().upper()
            ...
            try:
                ...
                for num in numeros:
                    if num in metodos_disponibles:
                        ...
                        print(f"⚠️ Método {num} no válido, ignorado")
            except:
                print("❌ Error en la selección, usando configuración básica")
                metodos_seleccionados = ['filtro_gaussiano', 'normalizacion', 'clahe']
        ...
        print(f"\nMétodos seleccionados: {', '.join(metodos_seleccionados)}")
        ...
        # Confirmación
        confirmacion = input(f"\n¿Procesar {len(imagenes)} imágenes con {len(metodos_seleccionados)} métodos? (s/N): ").strip().lower()
        if confirmacion not in ['s', 'sí', 'si', 'yes', 'y']:
            print("Operación cancelada")
            return
        ...
        try:
            ...
            print(f"❌ Error ejecutando procesamiento por lotes: {str(e)}")
            print("Verifique que todos los módulos estén correctamente configurados")
    ...
    def _mostrar_resultado_preprocesamiento(self, nombre_tecnica, imagen_resultado, modo_lotes=False, es_segmentacion=False, mascara_generada=None):
        ...
        Args:
            ...
            modo_lotes (bool): True si es procesamiento por lotes (sin previsualización)
            es_segmentacion (bool): True si es una operación de segmentación
            mascara_generada (np.ndarray): Máscara generada en el proceso (opcional)
        """
        ...
        if modo_lotes:
            # Modo lotes: solo guardar sin previsualización
            return self._procesar_resultado_lotes(nombre_tecnica, imagen_resultado)
        else:
            # Modo individual: con previsualización e interacción
            resultado = self._procesar_resultado_individual(nombre_tecnica, imagen_resultado)
            
            # Si es una operación de segmentación, ofrecer opciones adicionales
            if es_segmentacion:
    ...
    def _ofrecer_opciones_visualizacion_segmentacion(self, nombre_tecnica, imagen_resultado, mascara_generada=None):
        """
        Ofrece opciones de visualización específicas para segmentación.
        
        Args:
            nombre_tecnica: Nombre de la técnica de segmentación aplicada
            imagen_resultado: Imagen resultado de la segmentación
            mascara_generada: Máscara generada (si aplica)
        """
        print(f"\n🔍 OPCIONES DE VISUALIZACIÓN - {nombre_tecnica}")
        print("=" * 50)
        print("1. Ver segmentación aplicada sobre imagen original")
        print("2. Ver contornos de la segmentación sobre imagen original")
        print("3. Ambas opciones")
        print("0. Continuar sin visualización adicional")
        print("-" * 50)
        ...
        try:
            ...
            print(f"❌ Error en visualización de segmentación: {e}")
    
    def _mostrar_segmentacion_aplicada(self, nombre_tecnica, imagen_resultado, mascara_generada=None):
        """Muestra la segmentación aplicada sobre la imagen original."""
        try:
            # Determinar qué usar como máscara
            if mascara_generada is not None:
                mascara_a_usar = mascara_generada
            else:
                # Usar la imagen resultado como máscara
            ...
            print(f"✅ Mostrando segmentación aplicada de {nombre_tecnica}")
            ...
            try:
                ...
                plt.title(f'Segmentación Aplicada - {nombre_tecnica}')
                plt.axis('off')
                ...
                # Fallback a OpenCV si matplotlib no está disponible
                print("Usando visualización con OpenCV...")
                cv2.imshow('Imagen Original', self.imagen_actual)
                cv2.imshow(f'Segmentación Aplicada - {nombre_tecnica}', imagen_segmentada)
                cv2.waitKey(0)
                cv2.destroyAllWindows()
                
        except Exception as e:
            print(f"❌ Error mostrando segmentación aplicada: {e}")
    
    def _mostrar_contornos_segmentacion(self, nombre_tecnica, imagen_resultado, mascara_generada=None):
        """Muestra los contornos de la segmentación sobre la imagen original."""
        try:
            # Determinar qué usar como máscara
            if mascara_generada is not None:
                mascara_a_usar = mascara_generada
            ...
            print(f"✅ Mostrando contornos de {nombre_tecnica}")
            ...
            print(f"❌ Error mostrando contornos: {e}")
    
    def _procesar_resultado_individual(self, nombre_tecnica, imagen_resultado):
        """Procesa resultado en modo individual con previsualización."""
    ...
    def _procesar_resultado_lotes(self, nombre_tecnica, imagen_resultado):
        """Procesa resultado en modo lotes sin previsualización."""
        # En modo lotes, guardar automáticamente
        ruta_guardada = self._guardar_imagen_procesada(nombre_tecnica, imagen_resultado)
        if ruta_guardada:
            print(f"  → Imagen guardada: {os.path.basename(ruta_guardada)}")
        return imagen_resultado
    ...
    def _mostrar_comparacion_imagenes(self, imagen1, imagen2, titulo1, titulo2, auto_mostrar=False):
        ...
        Args:
            imagen1, imagen2: Imágenes a comparar
            titulo1, titulo2: Títulos para las imágenes
            auto_mostrar (bool): Si True, muestra automáticamente sin preguntar
        """
        mostrar = auto_mostrar
        
        if not auto_mostrar:
            opcion = input("¿Mostrar comparación visual? (s/N): ").strip().lower()
            mostrar = opcion in ['s', 'sí', 'si', 'yes', 'y']
        ...
        if mostrar:
            try:
                ...
                print(f"Error mostrando imágenes con matplotlib: {e}")
                print("Usando método alternativo con OpenCV...")
                # Método alternativo con OpenCV
                self._mostrar_comparacion_opencv(imagen1, imagen2, titulo1, titulo2)

    def _mostrar_comparacion_opencv(self, imagen1, imagen2, titulo1, titulo2):
        """
        Método alternativo usando OpenCV para mostrar comparación.
        """
        try:
            ...
            # Calcular tamaño máximo para visualización
            max_height = 600
            max_width = 800
            ...
            # Asegurar que ambas imágenes tengan la misma altura
            ...
            # Mostrar ventana
            window_name = "Comparación - Presiona cualquier tecla para continuar"
            cv2.namedWindow(window_name, cv2.WINDOW_NORMAL)
            cv2.imshow(window_name, imagen_combinada)
            
            print(f"\nComparación mostrada: {titulo1} vs {titulo2}")
            ...
            print(f"Error en visualización OpenCV: {e}")
            print("No se pudo mostrar la comparación visual")
   ...
   5. Revise resultados en ./resultados_deteccion/
    ...
    def aplicar_umbral_simple(self):
        ...
        try:
            ...
            self._mostrar_resultado_preprocesamiento("Umbralización Simple", resultado, es_segmentacion=True)
        except Exception as e:
    ...
    def aplicar_umbral_otsu(self):
        ...
        try:
            ...
            self._mostrar_resultado_preprocesamiento("Umbralización Otsu", resultado, es_segmentacion=True)
        except Exception as e:
    ...
    def aplicar_kmeans(self):
        ...
        try:
            ...
            self._mostrar_resultado_preprocesamiento("Segmentación K-means", resultado, es_segmentacion=True)
        except Exception as e:
    ...
    def aplicar_laplaciano_gauss(self):
        """Aplica filtro Laplaciano de Gauss (LoG) para detección de bordes."""
        try:
            if not self.verificar_imagen_cargada():
                return
            ...
            try:
                ...
                print("Valores inválidos, usando parámetros por defecto: sigma=2.0, umbral_factor=0.1")
                sigma = 2.0
                umbral_factor = 0.1
            ...
            print(f"❌ Error aplicando Laplaciano de Gauss: {e}")
            import traceback
    ...
    def aplicar_canny_segmentacion(self):
        ...
        try:
            ...
            self._mostrar_resultado_preprocesamiento("Segmentación Canny", resultado, es_segmentacion=True)
        except Exception as e:
            print(f"Error aplicando Canny: {e}")
    ...
    def detectar_contornos_segmentacion(self):
        ...
        try:
            ...
            self._mostrar_resultado_preprocesamiento("Detección de Contornos", resultado, es_segmentacion=True)
        except Exception as e:
    ...
    def segmentar_color_hsv(self):
        ...
        try:
            ...
            self._mostrar_resultado_preprocesamiento(f"Segmentación HSV (H:{hue_min}-{hue_max})", resultado, es_segmentacion=True)
    ...
    def segmentar_por_textura(self):
        ...
        try:
            ...
            self._mostrar_resultado_preprocesamiento(f"Segmentación Textura (ventana={tamano_ventana})", resultado, es_segmentacion=True)
    ...
    def aplicar_grabcut(self):
        ...
        try:
            ...
            # Convertir máscara a formato 0/255 para las funciones de visualización
    ...
    # =========================================================================
    # MÉTODOS PARA PROCESAMIENTO POR LOTES (SIN INTERACCIÓN DEL USUARIO)
    # =========================================================================
    
    def aplicar_filtro_gaussiano_lotes(self, imagen, sigma=1.0):
        """Aplica filtro gaussiano en modo lotes sin interacción."""
        if not self.verificar_imagen_cargada():
            return None
        try:
    ...
    def aplicar_normalizacion_lotes(self, imagen):
        """Aplica normalización en modo lotes sin interacción."""
        if not self.verificar_imagen_cargada():
            return None
        try:
            resultado = self.filtros.normalizar_imagen(imagen)
            return self._mostrar_resultado_preprocesamiento("Normalización", resultado, modo_lotes=True)
        except Exception as e:
            print(f"❌ Error en normalización: {e}")
            return None
    
    def aplicar_clahe_lotes(self, imagen, clip_limit=2.0):
        """Aplica CLAHE en modo lotes sin interacción."""
        if not self.verificar_imagen_cargada():
            return None
        try:
    ...
    def corregir_iluminacion_lotes(self, imagen):
        """Corrige iluminación en modo lotes sin interacción."""
        if not self.verificar_imagen_cargada():
            return None
        try:
            resultado = self.filtros.correccion_iluminacion(imagen)
            return self._mostrar_resultado_preprocesamiento("Corrección de Iluminación", resultado, modo_lotes=True)
        except Exception as e:
            print(f"❌ Error en corrección de iluminación: {e}")
            return None
    
    def reducir_ruido_bilateral_lotes(self, imagen, d=9, sigma_color=75, sigma_space=75):
        """Reduce ruido bilateral en modo lotes sin interacción."""
        if not self.verificar_imagen_cargada():
            return None
        try:
    ...
    def afilar_imagen_lotes(self, imagen):
        """Afila imagen en modo lotes sin interacción."""
        if not self.verificar_imagen_cargada():
            return None
        try:
            ...
            print(f"❌ Error en afilado: {e}")
            return None
    
    def redimensionar_imagen_lotes(self, imagen, nuevo_ancho=800, nuevo_alto=600):
        """Redimensiona imagen en modo lotes sin interacción."""
        if not self.verificar_imagen_cargada():
            return None
        try:
    ...
    def preprocesamiento_automatico_lotes(self, imagen):
        """Aplica preprocesamiento automático en modo lotes."""
        if not self.verificar_imagen_cargada():
            return None
        try:
            resultado = self.preprocessing.preprocesamiento_automatico(imagen)
            return self._mostrar_resultado_preprocesamiento("Preprocesamiento Automático", resultado, modo_lotes=True)
        except Exception as e:
            print(f"❌ Error en preprocesamiento automático: {e}")
            return None
    ...
    def aplicar_filtro_desenfoque_lotes(self, imagen, kernel_size=15):
        ...
        try:
            ...
            print(f"❌ Error en filtro de desenfoque: {e}")
            return None
    ...
    def ajustar_brillo_lotes(self, imagen, incremento=50):
        ...
        try:
            ...
            print(f"❌ Error en ajuste de brillo: {e}")
            return None
    ...
    def ajustar_contraste_lotes(self, imagen, factor=1.5):
        ...
        try:
            ...
            print(f"❌ Error en ajuste de contraste: {e}")
            return None
    ...
    def rotar_imagen_lotes(self, imagen, angulo=90):
        ...
        try:
            ...
            return self._mostrar_resultado_preprocesamiento("Rotación", resultado, modo_lotes=True)
        except Exception as e:
            print(f"❌ Error en rotación: {e}")
            return None
    
    def aplicar_umbral_simple_lotes(self, imagen, umbral=127):
        """Aplica umbralización simple en modo lotes."""
        if not self.verificar_imagen_cargada():
            return None
        try:
            resultado = self.segmentacion.umbral_simple(imagen, umbral)
            return self._mostrar_resultado_preprocesamiento("Umbralización Simple", resultado, modo_lotes=True)
        except Exception as e:
            print(f"❌ Error en umbralización simple: {e}")
            return None
    
    # =========================================================================
    # MÉTODOS HELPER PARA PROCESAMIENTO POR LOTES
    # =========================================================================
    
    def procesar_imagen_por_lotes(self, ruta_imagen, metodos_preprocesamiento=None):
        """
        Procesa una imagen individual en modo lotes con los métodos especificados.
        ...
        # Aplicar cada método de preprocesamiento
        imagen_trabajo = imagen_original.copy()
        
        for metodo in metodos_preprocesamiento:
            try:
                print(f"  → Aplicando {metodo}...")
                
                # Llamar al método correspondiente en modo lotes
                if metodo == 'filtro_gaussiano':
                    ...
                    print(f"⚠️ Método desconocido: {metodo}")
                    continue
    ...
    # =========================================================================
    # SISTEMA DE DETECCIÓN CON REDES NEURONALES
    # =========================================================================
    
    def sistema_deteccion_ia(self):
        """Maneja el sistema de detección con inteligencia artificial."""
        if not NEURAL_DETECTION_AVAILABLE:
            print("\n❌ Sistema de Detección con IA no disponible")
        ...
        try:
            ...
            if 'SistemaDeteccionSombrerosMejorado' in globals():
                ...
                # Ejecutar menú del sistema anterior
                while True:
                ...
                print("❌ No se pudo inicializar ningún sistema de IA")
                    
        except Exception as e:
            print(f"❌ Error en sistema de IA: {e}")
            print("💡 Verifique las dependencias de deep learning")
            input("\nPresiona Enter para continuar...")
    
    def mostrar_menu_sistema_ia(self):
        """Muestra el menú del sistema de IA."""
        print("\n" + "="*70)
        print("🧠 SISTEMA DE DETECCIÓN CON IA - MEJORADO".center(70))
        print("="*70)
        print("🎩 Detección de Sombreros con Redes Neuronales Avanzadas")
        print("-"*70)
        print("1. 🖼️  Detectar en Imagen Individual")
        print("2. 📂 Procesar Lote de Imágenes")
        print("3. 📹 Video en Tiempo Real")
        print("4. ⚙️  Configurar Modelos")
        print("5. 📊 Ver Estadísticas del Sistema")
        print("6. ❓ Ayuda del Sistema de IA")
        print("-"*70)
        print("🆕 FUNCIONALIDADES MEJORADAS:")
        print("   • Múltiples modelos (CNN, YOLO, ResNet, VGG)")
        print("   • Entrenamiento personalizado")
        print("   • Configuración avanzada de video")
    ...
    def ia_deteccion_imagen(self):
        ...
        if self.imagen_actual is None:
            print("\n❌ No hay imagen cargada")
            respuesta = input("¿Cargar una imagen? (s/N): ").lower().strip()
            if respuesta == 's':
                self.gestionar_imagenes()
                if self.imagen_actual is None:
                    return
        ...
        print(f"\n🎩 Detectando sombreros en: {os.path.basename(self.ruta_imagen_actual)}")
        ...
        print("4. Segmentación (U-Net, Mask R-CNN)")
        print("5. Todos los métodos")
        
        opcion = input("\nSeleccione método (1-5): ").strip()
        ...
        try:
            ...
            if resultado:
                ...
                print(f"   Métodos exitosos: {len(resumen['metodos_exitosos'])}")
    ...
    def ia_procesamiento_lote(self):
        ...
        directorio = input(f"\nDirectorio de imágenes ({self.directorio_imagenes}): ").strip()
        if not directorio:
        ...
        print("1. Solo YOLO (Rápido)")
        print("2. YOLO + Faster R-CNN")
        print("3. Todos los métodos")
        ...
        try:
            print(f"📂 Procesando lote desde: {directorio}")
            ...
            if resultado:
                print(f"\n📊 Resumen del procesamiento:")
                print(f"   Imágenes procesadas: {resultado['imagenes_procesadas']}")
                print(f"   Imágenes con detecciones: {resultado['imagenes_con_sombreros']}")
    ...
    def ia_video_tiempo_real(self):
        ...
        # Configurar salida
        guardar = input("¿Guardar video procesado? (s/N): ").lower().strip() == 's'
        output_path = None
        if guardar:
            timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    ...
    def ia_configuracion_modelos(self):
        """Configura y muestra información de modelos."""
        ...
        if opcion == '1':
            ...
            try:
                ...
                if 0.1 <= nuevo_umbral <= 0.9:
                    ...
                    print(f"✅ Umbral actualizado: {nuevo_umbral}")
                else:
                    print("❌ Umbral debe estar entre 0.1 y 0.9")
            except ValueError:
    ...
    def ia_estadisticas(self):
        """Muestra estadísticas del sistema de IA."""
    ...
    def ia_ayuda(self):
        ...
        print("\n" + "="*70)
        print("❓ AYUDA DEL SISTEMA DE DETECCIÓN CON IA")
        print("="*70)
        print("""
🎯 DESCRIPCIÓN:
   Sistema avanzado de detección de sombreros usando múltiples 
...
🧠 MODELOS DISPONIBLES:
   ...
   1. MODELOS PREENTRENADOS:
      ...
      • Faster R-CNN: Alta precisión, algo más lento
   ...
   2. REDES PERSONALIZADAS:
      ...
      • ResNet50/101: Skip connections para mejor gradiente
   
   3. SEGMENTACIÓN NEURONAL:
      • U-Net: Segmentación semántica pixel a pixel
      • Mask R-CNN: Segmentación de instancias individuales
      • DeepLabV3: Segmentación semántica avanzada

🚀 USO RECOMENDADO:
   
   • Para VELOCIDAD: Use YOLO v8
   • Para PRECISIÓN: Use Faster R-CNN
   • Para ANÁLISIS DETALLADO: Use segmentación con U-Net
   • Para COMPARACIÓN: Use "Todos los métodos"

📊 INTERPRETACIÓN DE RESULTADOS:
   
   • Confianza: 0.0-1.0 (mayor = más seguro)
   • Bbox: Coordenadas [x1, y1, x2, y2] del objeto
   • Área: Tamaño del objeto detectado en píxeles
   • Tiempo de inferencia: Velocidad del modelo

🎥 PROCESAMIENTO DE VIDEO:
   
   • Use solo YOLO para tiempo real (>20 FPS)
   • Faster R-CNN para análisis offline detallado
   • Presione 'q' para salir, 'p' para pausar

⚙️ CONFIGURACIÓN:
   
   • Umbral de confianza: Ajuste según necesidades
     - 0.3: Más detecciones, algunas falsas
     - 0.7: Menos detecciones, más precisas
   • Resolución de video afecta velocidad
        """)
        input("\nPresiona Enter para continuar...")
